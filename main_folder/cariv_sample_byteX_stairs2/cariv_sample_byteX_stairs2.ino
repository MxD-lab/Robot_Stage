/*
  ...
  2022/11/17（木）記入者：船橋佑｜移動量をかえるプログラムに変更
*/
...
bool serial_flag_a = false;
bool serial_flag_b = false;
bool serial_flag_c = false;
bool serial_flag_d = false;
bool serial_flag_e = false;
bool serial_flag_f = false;
bool serial_flag_g = false;
bool serial_flag_h = false;
...
void loop() {
  ...
  if(serial_flag3){
    ...
    serial_flag3 = false;
  }else if (serial_flag_a){
    x_position = move_to(stepper_x, 250, 100);
    Serial.println("a");
//    stepper_x.stop();
    serial_flag_a = false;
  }else if (serial_flag_b){
    x_position = move_to(stepper_x, 500, 100);
    Serial.println("b");
//    stepper_x.stop();
    serial_flag_b = false;
  }else if (serial_flag_c){
    x_position = move_to(stepper_x, 750, 100);
    Serial.println("c");
//    stepper_x.stop();
    serial_flag_c = false;
  }else if (serial_flag_d){
    x_position = move_to(stepper_x, 1000, 100);
    Serial.println("d");
//    stepper_x.stop();
    serial_flag_d = false;
  }else if (serial_flag_e){
    x_position = move_to(stepper_x, 1250, 100);
    Serial.println("e");
//    stepper_x.stop();
    serial_flag_e = false;
  }else if (serial_flag_f){
    x_position = move_to(stepper_x, 1500, 100);
    Serial.println("f");
//    stepper_x.stop();
    serial_flag_f = false;
  }else if (serial_flag_g){
    x_position = move_to(stepper_x, 1750, 100);
    Serial.println("g");
//    stepper_x.stop();
    serial_flag_g = false;
  }else if (serial_flag_h){
    x_position = move_to(stepper_x, 2000, 100);
    Serial.println("h");
//    stepper_x.stop();
    serial_flag_h = false;
    ...
    x_position = move_to(stepper_x, -800, -2000);
