...
void loop() {
    ...
    if (line.equals("move_z_start")) {
      ...
      while (true) {
        ...
        if (Serial.available()) {
          ...
          if (line2.equals("stop")) {
            stepper_z.stop();
          } else if (line2.equals("specify-Z")) {
            Serial.println("tomatta");
            break;
          }
        }
      }
  ...
  if (Serial.available()) {
    String line = Serial.readStringUntil(';');
    if (line.equals("move_x_start")) {
      stepper_x.moveTo(30000);
      stepper_x.setSpeed(2000);
      while (true) {
label_goto:
        stepper_x.runSpeedToPosition();
        if (Serial.available()) {
          ...
          if (line2.equals("adjust_Z-")) {
            ...
            while (true) {
              ...
              if (Serial.available()) {
                String line2 = Serial.readStringUntil(';');
                if (line2.equals("no-plom")) {
                  goto label_goto;
                }
              }
            }
          } else if (!line2.equals("adjust_Z+")) {
            stepper_z.setSpeed(500);
            while (true) {
              stepper_x.runSpeedToPosition();
              stepper_z.runSpeed();
...
//      stepper_z.stop();
//            stepper_z.moveTo(10000);
//            stepper_z.setSpeed(-2000);

//            while (true) {
//              stepper_z.runSpeedToPosition();
//              if (stepper_z.currentPosition() == 10000) {
//                stepper_z.stop();
//                Serial.println("move_z_end");
//              }
//            }
//            if (stepper_z.currentPosition() == 30000) {
//              stepper_z.stop();
//            break;
